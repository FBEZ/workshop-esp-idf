...
4. Connect via smartphone and verify the displayed page
...
* In the dropdown menu that appears → `Choose a container directory`
* Select a folder where the project folder will be created
* In the dropdown menu that appears → `template-app`
...
To disable NVS, open `menuconfig` and look for the `NVS` option:
...
We'll use `define` to set the parameters required by the softAP:
```c
...
To avoid overlapping with the other participants, please __choose and unique SSID name__.
...
* Create a function to handle the wifi events<br>
   _Since this function is called by `wifi_init_softap()`, you should put it before_
...
* Call the function inside the `app_main`<br>
   ...
   ```
* Choose target (`esp32c3`) and select port as done in the previous assignments. 
...
Take your smartphone, open the Wi-Fi list, and select the SSID you choose at the previous step (Fig.2)
