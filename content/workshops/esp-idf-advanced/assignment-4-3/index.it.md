...
title: "ESP-IDF Avanzato - Esercizio 4.3"
date: "2025-11-12"
series: ["WS00B"]
series_order: 16
showAuthor: false
...
In questo Esercizio abiliteremo la cifratura della flash.

{{< alert iconColor="#df8e1d" cardColor="#edcea3">}}
Abilitare la cifratura della flash (e il bootloader sicuro) è un’operazione irreversibile. Controllare attentamente ogni passaggio prima di procedere.
{{< /alert >}}

## Obiettivi dell’esercizio

1. Verificare lo stato di cifratura del dispositivo
2. Abilitare la cifratura della flash (modalità sviluppo)
3. Impostare la tabella delle partizioni
4. Verificare nuovamente lo stato di cifratura

## Verificare lo stato di cifratura del dispositivo

* Aprire un terminale ESP-IDF: `> ESP-IDF: Open ESP-IDF Terminal`
* All’interno del terminale, eseguire `idf.py efuse-summary`

Ora controlla gli eFuse rilevanti elencati nella tabella sottostante. Devono essere tutti nel loro stato predefinito (zero).

| **eFuse**                     | **Descrizione**                                                                                                                                                              |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BLOCK_KEYN`                  | Chiave AES. N è compreso tra 0 e 5.                                                                                                                           |
| `KEY_PURPOSE_N`               | Controlla il blocco eFuse `BLOCK_KEYN`, dove N è tra 0 e 5.                          |
| `DIS_DOWNLOAD_MANUAL_ENCRYPT` | Disabilita la cifratura flash quando si utilizza la modalità di avvio download.                                                                             |
| `SPI_BOOT_CRYPT_CNT`          | Abilita cifratura quando è impostata una modalità di avvio SPI. La funzione è abilitata se 1 o 3 bit sono impostati nell’eFuse. |


<details>
<summary>Mostra i blocchi eFuse</summary>
...
## Abilitare la cifratura della flash

* Aprire `menuconfig`: `> ESP-IDF: SDK Configuration Editor (menuconfig)`<br>
  → `Security Features` → `Enable flash encryption on boot (READ DOCS FIRST)`

* Assicurarsi di avere: `Enable usage mode` → `Development (Not secure)`

Dovresti avere una configurazione simile alla seguente:

{{< figure
default=true
src="/workshop-esp-idf/workshops/esp-idf-advanced/assets/assignment_4_2_flash_encryption.webp"
height=500
caption="Opzioni di cifratura della flash"
>}}

Aumentiamo la verbosità del bootloader per vedere cosa succede:

* Aprire `menuconfig`: `> ESP-IDF: SDK Configuration Editor (menuconfig)`<br>
  → `Bootloader log verbosity` → `verbose`

## Impostare la tabella delle partizioni

Se provassi a flashare il progetto, otterresti un errore. 

<details>
<summary>Mostra errore</summary>
...
Il motivo è che il bootloader ora occupa più spazio e non rientra nello spazio del offset standard della tabella delle partizioni (0x8000). È necessario cambiarlo a `0xf000` prima di procedere.

* Apri `menuconfig`: `> ESP-IDF: SDK Configuration Editor (menuconfig)`<br>
  → `Offset of partition table` → `0xf000`
...
I log di flashing saranno diversi dal solito questa volta, informandoci sul processo di cifratura della flash.
{{< /alert >}}


<details>
<summary>Mostra i log</summary>
...
Dovete completare gli ultimi due passaggi:

* Premere il pulsante di reset sul dispositivo
* Caricare di nuovo il progetto: `> ESP-IDF: Flash (UART) Your Project`

### Verificare nuovamente lo stato di cifratura del dispositivo
...
<details>
<summary>Mostra estratto degli eFuses</summary>
...
A questo punto, dovresti identificare le seguenti differenze:
...
* La cifratura della flash è attiva (`SPI_BOOT_CRYPT_CNT`).
* Uno dei blocchi eFuse è stato riservato per memorizzare la chiave di cifratura.

Ora il vostro dispositivo ha la flash cifrata. Siccome abbiamo selezionato la modalità sviluppo, è ancora possibile ricaricarlo tramite la porta seriale.

## Conclusione

In questo Esercizio, abbiamo aggiunto la cifratura della flash al progetto abilitando le opzioni appropriate in `menuconfig` e adattando l’offset della tabella delle partizioni come richiesto.
