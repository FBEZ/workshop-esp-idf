...
In the previous assignment code, we were already indirectly using this event loop for capturing MQTT and Wi-Fi events.
{{< /alert >}}
...
4. Create the two timers<br>
   * `esp_timer_create`
   * `esp_timer_start_periodic`
5. Create the timer callback functions to post the event every 5s and 200ms<br>
   * `temp_timer_callback`
   * `alarm_timer_callback`
6. Create a infinite sleep loop in main
...
* Define their `event_id` as an enum (in this case, both comprise just a single value):
    ```c
    typedef enum {
        TEMP_EVENT_MEASURE,
    } temp_event_id_t;
...
* Create and start the temperature and alarm timers:
    ```c
    // Create and start periodic timers (app_main)
...
* For the sake of semplicity, define them at the beginning of `app_main.c` as
    ```c
