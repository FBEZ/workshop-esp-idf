---
title: "ESP-IDF Avanzato - Esercizio 1.1"
date: "2025-11-12"
series: ["WS00B"]
series_order: 3
showAuthor: false
summary: "Crea il componente `alarm` e rifattorizza il codice per utilizzarlo. (Guidato)"
---
...
## Scaricare il codice `assignment_1_1_base`
...
* Apri la cartella `assignment_1_1_base` con VS Code<br>
{{< alert icon="circle-info" cardColor="#b3e0f2" iconColor="#04a5e5">}}
Delle cartelle nella repo, `assigment_1_1_base` e `assignment_3_2_base` contengono il codice di partenza per i rispettivi esercizi 1.1 e 3.2. Tutte le altre cartelle contengono invece la soluzione delle esercitazioni.
{{< /alert >}}

* Imposta il target:  `> ESP-IDF: Set Espressif Device Target`
* Seleziona la porta: `> ESP-IDF: Select Port to Use (COM, tty, usbserial)`
* Imposta i dati dell’AP:<br>

  * Apri il `menuconfig`: `> ESP-IDF: SDK Configuration Editor (menuconfig)`<br>
    → `WiFi SSID` &rarr; Imposta l'SSID del hotspot<br>
    → `WiFi Password` &rarr; Imposta la password<br>
* Compila, carica il binario e monitora il dispositivo<br>
...
### Creare il componente `alarm`

Per creare il componente `alarm`, devi:

1. `> ESP-IDF: Create New ESP-IDF Component`<br>
   Nel menù a tendina inserisci: `alarm`<br>
   Verrà creata una nuova cartella `components/alarm`.
2. Sposta i file nella cartella del componente<br>
   `alarm.c` → `components/alarm/alarm.c`<br>
   `alarm.h` → `components/alarm/include/alarm.h`
3. Aggiungi la dipendenza `esp_timer` nel file `CMakeLists.txt` del componente<br>
...
{{< alert icon="circle-info" cardColor="#b3e0f2" iconColor="#04a5e5">}}
Non è necessario aggiungere `REQUIRES esp_timer` nel componente main. Il main ha accesso di default a tutti i componenti disponibili. 
{{< /alert >}}
4. Rimuovi il sorgente da `main/CMakeLists.txt`<br>
   ```console
   idf_component_register(SRCS "app_main.c"
                    INCLUDE_DIRS ".")
   ```
5. Fai un full clean: `> ESP-IDF: Full Clean Project`

6. Ricompila: `> ESP-IDF: Build, Flash and Start a Monitor on Your Device`

Se tutto è andato a buon fine, possiamo passare ora alla creazione di una configurazione del componente. 

### Aggiungere opzioni alla configurazione del componente

* Apri il file `components/alarm/alarm.c`
* Al suo interno puoi notare due valori hardcoded:<br>
...
Sostituiremo questi valori con delle configurazioni del componente.

* Crea un file `Kconfig` nella directory principale del componente `alarm`
* Aggiungi il seguente contenuto al file `Kconfig`<br>
   ...
   endmenu
   ```
2. Commenta o cancella le define<br>

   ```c
   //#define ALARM_THRESHOLD_PERCENT     2     // 2% chance
   //#define ALARM_REFRESH_INTERVAL_MS   1000   // reevaluate every 1000 ms
   ```
3. Sostituisci i nomi delle macro nel resto del codice<br>

   * `ALARM_THRESHOLD_PERCENT` → `CONFIG_ALARM_THRESHOLD_PERCENT`
   * `ALARM_REFRESH_INTERVAL_MS` → `CONFIG_ALARM_REFRESH_INTERVAL_MS`
4. Pulisci il progetto: `ESP-IDF: Full Clean Project`
5. Ricompila e riprogramma: `ESP-IDF: Build, Flash and Start Monitor on Your Device`
...
// #define ALARM_THRESHOLD_PERCENT     20     // 2% chance
// #define ALARM_REFRESH_INTERVAL_MS   1000   // reevaluate every 1000 ms
